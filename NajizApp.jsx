...
const GUMROAD_LINKS = {
  PMP: "https://gumroad.com/l/YOUR-PRODUCT-LINK", // ⚠️ بدّل ده برابط منتجك الحقيقي
};
...
export default function NajizApp(){
  ...
  return (
    <div dir="rtl" className={`theme-${theme}`} style={{minHeight:"100vh",background:COLORS.bg,fontFamily:"'Cairo',sans-serif",color:COLORS.light,transition:"background .2s ease,color .2s ease"}}>
      ...
      {view==="dashboard" && <Dashboard progress={progress} setExamDate={setExamDate} setView={setView} profile={profile} />}
      {view==="profile" && <ProfileScreen profile={profile} loading={profileLoading} saveProfile={saveProfile} progress={progress} userId={userId} setView={setView} />}
...
/* ============================= الملف الشخصي ============================= */
function ProfileScreen({ profile, loading, saveProfile, progress, userId, setView }){
  ...
  useEffect(()=>{
    if(!userId) return;
    (async ()=>{
      const { data: pData } = await supabase.from("purchases").select("*").eq("user_id", userId).order("created_at",{ascending:false});
      const pByCode = {};
      (pData||[]).forEach(row=>{ if(!pByCode[row.cert_code]) pByCode[row.cert_code]=row; });
      setPurchases(pByCode);
    })();
  },[userId]);
  ...
  // تقرير الجاهزية لبنك PMP (البنك الحقيقي الوحيد حاليًا)
  ...
  const uploadReceipt = async (certCode, file) => {
    ...
    if(!error) setPurchases(p=>({...p, [certCode]: data}));
  };
...
/* ============================= لوحة التحكم ============================= */
function Dashboard({ progress, setExamDate, setView, profile }){
