    ...
    <div dir="rtl" className={`theme-${theme}`} style={{minHeight:"100vh",background:COLORS.bg,fontFamily:"'Cairo',sans-serif",color:COLORS.light,transition:"background .2s ease,color .2s ease"}}>
      ...
      {view==="courses" && (
        <CourseCatalog courses={courses} loading={coursesLoading} purchases={coursePurchases}
          ...
          onOpenCourse={(id)=>{ setSelectedCourseId(id); setView("coursePlayer"); }} />
      )}
      {view==="coursePlayer" && (
        <CoursePlayer courseId={selectedCourseId} course={courses?.find(c=>c.id===selectedCourseId)}
          purchased={coursePurchases[selectedCourseId]?.status==="approved"} userId={userId}
...
function LessonManager({ courseId }){
  const [lessons, setLessons] = useState(null);
  const [newLesson, setNewLesson] = useState({ title:"", video_url:"", duration_minutes:10, is_preview:false });
  const [saving, setSaving] = useState(false);
  ...
  const addLesson = async (e) => {
    ...
    setNewLesson({ title:"", video_url:"", duration_minutes:10, is_preview:false });
    load();
  };
        ...
        <div style={{display:"flex",flexDirection:"column",gap:6}}>
          {lessons.map((l,i)=>(
            <div key={l.id} style={{display:"flex",alignItems:"center",gap:10,padding:"8px 10px",background:"var(--panel2)",borderRadius:8}}>
              <span style={{fontSize:12.5,flex:1}}>{i+1}. {l.title} {l.is_preview && <span style={{color:COLORS.teal,fontSize:10.5}}>(معاينة)</span>}</span>
...
/* ============================= مشغّل الكورس ============================= */
function CoursePlayer({ courseId, course, purchased, userId, onExit }){
  const { lessons, loading } = useLessons(courseId);
  const { done, markDone } = useLessonProgress(userId);
              ...
              {lessons.map((l,i)=>{
                ...
                return (
                  <button key={l.id} onClick={()=>setActiveLessonId(l.id)} style={{
                    ...
                    <span style={{fontSize:12.5,color: isActive?COLORS.light:COLORS.sub,flex:1}}>{i+1}. {l.title}</span>
