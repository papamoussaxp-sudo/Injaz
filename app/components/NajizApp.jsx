...
function useProfile(userId){
  ...
  useEffect(()=>{
    ...
    (async ()=>{
      ...
      if(cancelled) return;
      setProfile(data || { user_id:userId, full_name:"", birth_date:"", phone:"", selected_certs:[], selected_courses:[] });
      setLoading(false);
    })();
    return ()=>{ cancelled = true; };
  },[userId]);
...
export default function NajizApp(){
  ...
  const PROTECTED_VIEWS = ["dashboard","practice","review","mock","flashcards","profile","courses"];
  useEffect(()=>{
  ...
  // "السلة": بيسجّل اهتمام المستخدم بكورس معيّن في بروفايله عشان يظهر في ملفه الشخصي حتى لو لسه ما اشتراش،
  // ولو الزائر مش مسجّل دخول، بنودّيه لتسجيل الدخول الأول قبل ما نكمل عملية الشراء
  const addCourseToCart = async (courseId) => {
    if(!session){ setView("auth"); return false; }
    const current = profile?.selected_courses || [];
    if(!current.includes(courseId)){
      await saveProfile({ ...(profile||{}), selected_courses: [...current, courseId] });
    }
    return true;
  };
  ...
  return (
    <div dir="rtl" className={`theme-${theme}`} style={{minHeight:"100vh",background:COLORS.bg,fontFamily:"'Cairo',sans-serif",color:COLORS.light,transition:"background .2s ease,color .2s ease"}}>
      <FontLoader />
      <GlobalStyle />
      {view!=="landing" && !(view==="coursePlayer" && coursePurchases[selectedCourseId]?.status!=="approved") && (
      ...
      {view==="landing" && <Landing onStart={goStart} onTrial={startTrial} certList={certList} certsLoading={certsLoading} courses={courses} coursesLoading={coursesLoading} onOpenCourse={(id)=>router.push(`/courses/${id}`)} />}
      ...
      {view==="profile" && (
        <ProfileScreen profile={profile} loading={profileLoading} saveProfile={saveProfile} progress={progress} certList={certList}
          ...
          courses={courses} coursePurchases={coursePurchases} onCoursePurchaseAdded={addCoursePurchaseLocal} onOpenCourse={(id)=>router.push(`/courses/${id}`)} />
      )}
      {view==="practice" && (
      ...
      {view==="courses" && (
        <CourseCatalog courses={courses} loading={coursesLoading} purchases={coursePurchases}
          userId={userId} onPurchaseAdded={addCoursePurchaseLocal} onAddToCart={addCourseToCart}
          onOpenCourse={(id)=>{ router.push(`/courses/${id}`); }} />
      )}
      {view==="coursePlayer" && (
        coursePurchases[selectedCourseId]?.status==="approved" ? (
          <CoursePlayer courseId={selectedCourseId} course={courses?.find(c=>c.id===selectedCourseId)}
            purchased={true} userId={userId} profile={profile} saveProfile={saveProfile}
            onExit={()=>setView("courses")} />
        ) : (
          <CourseDetailsPage
            courseId={selectedCourseId} course={courses?.find(c=>c.id===selectedCourseId)}
            userId={userId} isLoggedIn={!!session} userEmail={session?.user?.email} purchase={coursePurchases[selectedCourseId]}
            onPurchaseAdded={addCoursePurchaseLocal}
            onAddToCart={addCourseToCart}
            onStartWatching={()=>{}}
            onLogin={()=>setView("auth")}
            onGoDashboard={()=>setView("dashboard")}
            onBack={()=>setView(session ? "courses" : "landing")}
          />
        )
      )}
    </div>
...
const EMPTY_COURSE_FORM = { title:"", description:"", price_usd:79, thumbnail_url:"", gumroad_url:"", instructor_name:"", instructor_photo_url:"", objectives:"", target_audience:"", skills:"", exam_details:"" };
...
function CourseFormFields({ form, setForm, uploadingThumb, uploadingInstructor, onUploadThumb, onUploadInstructor }){
  ...
  return (
    <>
      ...
      <textarea placeholder="وصف مختصر (نظرة عامة تظهر أول تاب في صفحة تفاصيل الكورس)" value={form.description} onChange={e=>f("description",e.target.value)} style={{...fieldStyle,minHeight:70}}/>
      ...
      <div style={{borderTop:"1px dashed var(--border)",paddingTop:14}}>
        <div style={{fontWeight:700,fontSize:13,marginBottom:4}}>محتوى صفحة تفاصيل الكورس (تظهر كتابات للطالب قبل الشراء)</div>
        <div style={{fontSize:11.5,color:COLORS.sub,marginBottom:10}}>اكتب كل نقطة في سطر منفصل — كل سطر هيتحوّل تلقائيًا لعنصر منفصل في القائمة</div>

        <label style={{fontSize:12,color:COLORS.sub,display:"block",marginBottom:5}}>الأهداف المتوقع تحقيقها من الدورة</label>
        <textarea placeholder={"فهم إطار عمل PMI\nتطبيق مفاهيم حوكمة المشاريع\n..."} value={form.objectives} onChange={e=>f("objectives",e.target.value)} style={{...fieldStyle,minHeight:90,fontFamily:"inherit"}}/>

        <label style={{fontSize:12,color:COLORS.sub,display:"block",marginBottom:5}}>من يحتاج هذه الدورة</label>
        <textarea placeholder={"مديرو المشاريع\nالمهندسون والمتخصصون في الإنشاءات\n..."} value={form.target_audience} onChange={e=>f("target_audience",e.target.value)} style={{...fieldStyle,minHeight:80,fontFamily:"inherit"}}/>

        <label style={{fontSize:12,color:COLORS.sub,display:"block",marginBottom:5}}>المهارات والكفاءات المكتسبة</label>
        <textarea placeholder={"بدء المشاريع بصورة سليمة\nإدارة نطاق المشروع باستخدام WBS\n..."} value={form.skills} onChange={e=>f("skills",e.target.value)} style={{...fieldStyle,minHeight:90,fontFamily:"inherit"}}/>

        <label style={{fontSize:12,color:COLORS.sub,display:"block",marginBottom:5}}>تفاصيل الاختبار (شروط الدخول، الصيغة، عدد الأسئلة...)</label>
        <textarea placeholder={"مجموع الأسئلة 180 سؤالًا\nمدة الاختبار 230 دقيقة\n..."} value={form.exam_details} onChange={e=>f("exam_details",e.target.value)} style={{...fieldStyle,minHeight:100,fontFamily:"inherit"}}/>
      </div>
    </>
  );
}
...
function CourseManager(){
  ...
  const startEdit = (c) => {
    ...
    setEditForm({
      ...
      objectives:c.objectives||"", target_audience:c.target_audience||"", skills:c.skills||"", exam_details:c.exam_details||"",
    });
    setExpandedId(null);
  };
...
/* ============================= الملف الشخصي ============================= */
function ProfileScreen({ profile, loading, saveProfile, progress, userId, setView, purchases, onPurchaseAdded, certList=[], courses=[], coursePurchases={}, onCoursePurchaseAdded, onOpenCourse }){
  const [form, setForm] = useState(null);
  const [editing, setEditing] = useState(false);
  const [saved, setSaved] = useState(false);
  const [saving, setSaving] = useState(false);
  const [uploadStatus, setUploadStatus] = useState({}); // certCode -> "uploading" | "done" | "error"
  const [courseUploadStatus, setCourseUploadStatus] = useState({}); // courseId -> "uploading" | "done" | "error"
  ...
  const uploadCourseReceipt = async (courseId, file) => {
    if(!file || !userId) return;
    setCourseUploadStatus(s=>({...s, [courseId]:"uploading"}));
    const path = `${userId}/course-receipt-${courseId}-${Date.now()}-${file.name}`;
    const { error: upErr } = await supabase.storage.from("receipts").upload(path, file);
    if(upErr){ setCourseUploadStatus(s=>({...s, [courseId]:"error"})); return; }
    ...
    if(!error){
      onCoursePurchaseAdded && onCoursePurchaseAdded(courseId, data);
      setCourseUploadStatus(s=>({...s, [courseId]:"done"}));
    } else {
      setCourseUploadStatus(s=>({...s, [courseId]:"error"}));
    }
  };

  // نعرض في "كورساتي" أي كورس المستخدم مهتم بيه (سلة) أو اشتراه فعلًا حتى لو مش في السلة
  const cartCourseIds = form.selected_courses || [];
  const purchasedCourseIds = Object.keys(coursePurchases||{});
  const myCourseIds = Array.from(new Set([...cartCourseIds, ...purchasedCourseIds]));
  const myCourses = myCourseIds.map(id=>courses.find(c=>c.id===id)).filter(Boolean);
  ...
  return (
    <div style={{maxWidth:680,margin:"0 auto",padding:"30px 20px 60px"}}>
      ...
      {/* تقرير الكورسات */}
      <div style={{fontWeight:700,fontSize:16,margin:"28px 0 14px"}}>كورساتي</div>
      {myCourses.length===0 ? (
        <div className="card" style={{padding:24,textAlign:"center",color:COLORS.sub,fontSize:13.5}}>
          لسه ما ضفتش أي كورس. تصفّح <button onClick={()=>setView("courses")} style={{background:"none",border:"none",color:COLORS.teal,fontWeight:700,cursor:"pointer",fontFamily:"'Cairo',sans-serif",fontSize:13.5,padding:0}}>الكورسات</button> واختار اللي يناسبك.
        </div>
      ) : (
        <div style={{display:"flex",flexDirection:"column",gap:14}}>
          {myCourses.map(course=>{
            const purchase = coursePurchases[course.id];
            const isApproved = purchase?.status==="approved";
            const isPending = purchase?.status==="pending";
            return (
              <div key={course.id} className="card" style={{padding:20}}>
                <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:14}}>
                  <Video size={19} color={COLORS.teal}/>
                  <div style={{fontWeight:700,fontSize:15,flex:1}}>{course.title}</div>
                  ...
                  {isPending && <span style={{fontSize:11,fontWeight:700,color:COLORS.gold,background:"rgba(255,200,87,0.12)",padding:"4px 10px",borderRadius:8}}>قيد المراجعة</span>}
                  {!purchase && <span style={{fontSize:11,fontWeight:700,color:COLORS.sub,background:"var(--panel2)",padding:"4px 10px",borderRadius:8}}>لسه ما اتفعلش</span>}
                </div>

                <div style={{padding:"12px 14px",borderRadius:10,background:"var(--panel2)",display:"flex",alignItems:"center",gap:10,flexWrap:"wrap"}}>
                  {isApproved && (
                    <><CheckCircle2 size={16} color={COLORS.teal}/><span style={{fontSize:13,color:COLORS.teal,fontWeight:700}}>تم تفعيل الشراء ✓</span>
                    <button className="btn btn-primary" style={{marginRight:"auto",fontSize:12.5,padding:"7px 14px"}} onClick={()=>onOpenCourse && onOpenCourse(course.id)}>ابدأ المشاهدة</button></>
                  )}
                  {isPending && (
                    <><Hourglass size={16} color={COLORS.gold}/><span style={{fontSize:13,color:COLORS.gold}}>استلمنا إيصالك، بنراجعه ونفعّل خلال ساعات</span></>
                  )}
                  {!purchase && (
                    <div style={{display:"flex",flexDirection:"column",gap:10,width:"100%"}}>
                      <div style={{fontSize:13,color:COLORS.sub}}>لسه ما اشتريتش كورس {course.title} (${course.price_usd}) — اشترِ وارفع إيصال الدفع عشان يتفعّل.</div>
                      <div style={{display:"flex",gap:10,flexWrap:"wrap",alignItems:"center"}}>
                        {course.gumroad_url && (
                          <a href={course.gumroad_url} target="_blank" rel="noopener noreferrer" className="btn btn-gold" style={{textDecoration:"none",fontSize:13}}>اشترِ الآن — ${course.price_usd}</a>
                        )}
                        <label className="btn btn-outline" style={{fontSize:13,cursor:"pointer",display:"flex",alignItems:"center",gap:6,opacity:courseUploadStatus[course.id]==="uploading"?0.6:1}}>
                          {courseUploadStatus[course.id]==="uploading" ? <Loader2 size={14} className="spin"/> : <Upload size={14}/>}
                          {courseUploadStatus[course.id]==="uploading" ? "جاري الرفع..." : "ارفع إيصال الدفع"}
                          <input type="file" accept="image/*,application/pdf" style={{display:"none"}} disabled={courseUploadStatus[course.id]==="uploading"}
                            onChange={e=>{ if(e.target.files[0]) uploadCourseReceipt(course.id, e.target.files[0]); }}/>
                        </label>
                        {courseUploadStatus[course.id]==="done" && (
                          <span style={{fontSize:12.5,color:COLORS.teal,fontWeight:700,display:"flex",alignItems:"center",gap:5}}><CheckCircle2 size={15}/> تم رفع الإيصال بنجاح ✓</span>
                        )}
                        {courseUploadStatus[course.id]==="error" && (
                          <span style={{fontSize:12.5,color:"#ff8a8a",fontWeight:700,display:"flex",alignItems:"center",gap:5}}><AlertCircle size={15}/> حصل خطأ، جرب تاني</span>
                        )}
                      </div>
                    </div>
...
function CoursesTeaser({ courses, loading, onStart, onOpenCourse }){
  return (
              ...
              <div key={c.id} className="card" style={{overflow:"hidden"}}>
                ...
                <div style={{padding:18}}>
                  ...
                  <div style={{display:"flex",alignItems:"center",justifyContent:"space-between"}}>
                    ...
                    <button className="btn btn-outline" style={{fontSize:12.5,padding:"7px 14px"}} onClick={()=>onOpenCourse ? onOpenCourse(c.id) : onStart()}>التفاصيل</button>
                  </div>
                </div>
              </div>
            ))}
...
function Landing({ onStart, onTrial, certList=[], certsLoading, courses, coursesLoading, onOpenCourse }){
  return (
    <div>
      {/* Top bar */}
      <div style={{position:"sticky",top:0,zIndex:20,background:"var(--navbg)",backdropFilter:"blur(10px)",borderBottom:"1px solid var(--border)"}}>
      ...
      <CoursesTeaser courses={courses} loading={coursesLoading} onStart={onStart} onOpenCourse={onOpenCourse} />
...
/* ============================= كتالوج الكورسات ============================= */
function CourseCatalog({ courses, loading, purchases, userId, onPurchaseAdded, onOpenCourse, onAddToCart }){
                ...
                <div style={{padding:18}}>
                  ...
                  {isApproved ? (
                    ...
                    <div style={{display:"flex",flexDirection:"column",gap:8}}>
                      <button className="btn" style={{width:"100%",background:"var(--panel2)",fontSize:13}} onClick={()=>onOpenCourse(c.id)}>التفاصيل</button>
                      {c.gumroad_url && (
                        <button className="btn btn-gold" style={{fontSize:13}} disabled={buyingId===c.id} onClick={async()=>{
                          setBuyingId(c.id);
                          const ok = await onAddToCart(c.id);
                          setBuyingId(null);
                          if(ok) window.open(c.gumroad_url, "_blank", "noopener,noreferrer");
                        }}>اشترِ الآن</button>
                      )}
...
/* ============================= صفحة تفاصيل الكورس (متاحة للجميع حتى بدون شراء) ============================= */
const linesToList = (text) => (text||"").split("\n").map(s=>s.trim()).filter(Boolean);

function CourseDetailsPage({ course, courseId, userId, isLoggedIn, userEmail, purchase, onPurchaseAdded, onAddToCart, onStartWatching, onLogin, onGoDashboard, onBack }){
  ...
  const uploadReceipt = async (file) => {
    ...
    const path = `${userId}/course-receipt-${courseId}-${Date.now()}-${file.name}`;
    const { error: upErr } = await supabase.storage.from("receipts").upload(path, file);
    if(upErr){ setUploadStatus("error"); return; }
    const { data, error } = await supabase.from("course_purchases").insert({
      user_id: userId, course_id: courseId, receipt_path: path, status: "pending"
    }).select().single();
  ...
  const handleBuyClick = async () => {
    setBuying(true);
    const ok = await onAddToCart(courseId); // بيسجّل الكورس في سلة/بروفايل المستخدم، وبيودّي لتسجيل الدخول لو زائر
    setBuying(false);
    if(ok && course?.gumroad_url) window.open(course.gumroad_url, "_blank", "noopener,noreferrer");
  };

  const TABS = course ? [
    { id:"overview", label:"نظرة عامة", show: !!course.description },
    { id:"objectives", label:"الأهداف", show: linesToList(course.objectives).length>0 },
    { id:"audience", label:"لمن هذه الدورة", show: linesToList(course.target_audience).length>0 },
    { id:"skills", label:"المهارات والكفاءات", show: linesToList(course.skills).length>0 },
    { id:"exam", label:"تفاصيل الاختبار", show: !!course.exam_details },
    { id:"syllabus", label:"محتوى الدورة", show: true },
  ].filter(t=>t.show) : [];
  ...
  return (
    <div dir="rtl" className="theme-light" style={{minHeight:"100vh",background:COLORS.bg,fontFamily:"'Cairo',sans-serif",color:COLORS.light}}>
      <FontLoader /><GlobalStyle />

      {/* هيدر مستقل بنفس هوية الصفحة الرئيسية — مش جزء من لوحة التحكم */}
      <div style={{position:"sticky",top:0,zIndex:20,background:"var(--navbg)",backdropFilter:"blur(10px)",borderBottom:"1px solid var(--border)"}}>
        <div style={{maxWidth:1160,margin:"0 auto",display:"flex",alignItems:"center",justifyContent:"space-between",padding:"12px 20px"}}>
          <div style={{display:"flex",alignItems:"center",gap:14}}>
            <button onClick={onBack} style={{background:"none",border:"none",color:COLORS.sub,cursor:"pointer",display:"flex",alignItems:"center",gap:4,fontFamily:"'Cairo',sans-serif",fontSize:13}}><ArrowLeft size={15}/> رجوع</button>
            <Logo />
          </div>
          <div style={{display:"flex",alignItems:"center",gap:10}}>
            {isLoggedIn ? (
              <button className="btn btn-outline" style={{padding:"9px 16px",fontSize:13.5}} onClick={onGoDashboard}>لوحة التحكم</button>
            ) : (
              <button className="btn btn-primary" style={{padding:"9px 18px",fontSize:13.5}} onClick={onLogin}>تسجيل الدخول</button>
            )}
          </div>
        </div>
      </div>
      ...
      {!course ? (
        <LoadingBlock text="جاري تحميل تفاصيل الكورس..." />
      ) : (
      <div style={{maxWidth:1000,margin:"0 auto",padding:"28px 20px 60px"}}>
        {course.thumbnail_url && (
          <div style={{height:220,borderRadius:16,marginBottom:20,background:`url(${course.thumbnail_url}) center/cover`,border:"1px solid var(--border)"}}/>
        )}

        <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",flexWrap:"wrap",gap:16,marginBottom:24}}>
          <div style={{flex:1,minWidth:260}}>
            <h1 style={{fontSize:24,fontWeight:800,margin:"0 0 10px"}}>{course.title}</h1>
            {course.instructor_name && (
              <div style={{display:"flex",alignItems:"center",gap:10}}>
                {course.instructor_photo_url ? (
                  <img src={course.instructor_photo_url} alt={course.instructor_name} style={{width:34,height:34,borderRadius:"50%",objectFit:"cover"}}/>
                ) : (
                  <div style={{width:34,height:34,borderRadius:"50%",background:"var(--panel2)",display:"flex",alignItems:"center",justifyContent:"center"}}><UserCircle size={19} color={COLORS.sub}/></div>
                )}
                <span style={{fontSize:13.5,color:COLORS.sub}}>{course.instructor_name}</span>
              </div>
            )}
          </div>

          <div className="card" style={{padding:18,minWidth:220,textAlign:"center"}}>
            {isApproved ? (
              <>
                <div style={{fontSize:13,color:COLORS.teal,fontWeight:700,marginBottom:12,display:"flex",alignItems:"center",justifyContent:"center",gap:6}}><CheckCircle2 size={16}/> الكورس مفعّل عندك</div>
                <button className="btn btn-primary" style={{width:"100%"}} onClick={onStartWatching}><PlayCircle size={16} style={{verticalAlign:"middle",marginLeft:6}}/> ابدأ المشاهدة</button>
              </>
            ) : isPending ? (
              <div style={{fontSize:13,color:COLORS.gold}}><Hourglass size={16} style={{verticalAlign:"middle",marginLeft:6}}/> بنراجع إيصالك، هيتفعّل خلال ساعات</div>
            ) : (
              <>
                <div style={{fontWeight:800,fontSize:24,marginBottom:12}}>${course.price_usd}</div>
                <div style={{display:"flex",flexDirection:"column",gap:8}}>
                  {course.gumroad_url && (
                    <button className="btn btn-gold" disabled={buying} onClick={handleBuyClick}>
                      <ShoppingCart size={15} style={{verticalAlign:"middle",marginLeft:6}}/> {isLoggedIn ? "اشترِ الآن — أضِفه لسلتي" : "سجّل دخولك للشراء"}
                    </button>
                  )}
                  {isLoggedIn && (
                    <label className="btn btn-outline" style={{fontSize:12.5,cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center",gap:6,opacity:uploadStatus==="uploading"?0.6:1}}>
                      {uploadStatus==="uploading" ? <Loader2 size={13} className="spin"/> : <Upload size={13}/>}
                      {uploadStatus==="uploading" ? "جاري الرفع..." : "ارفع إيصال الدفع"}
                      <input type="file" accept="image/*,application/pdf" style={{display:"none"}} disabled={uploadStatus==="uploading"}
                        onChange={e=>{ if(e.target.files[0]) uploadReceipt(e.target.files[0]); }}/>
                    </label>
                  )}
                  {uploadStatus==="done" && <div style={{fontSize:11,color:COLORS.teal}}>✓ تم رفع الإيصال بنجاح</div>}
                  {uploadStatus==="error" && <div style={{fontSize:11,color:"#ff8a8a"}}>حصل خطأ، حاول تاني</div>}
                </div>
              </>
            )}
          </div>
        ...
        {/* التابات */}
        <div style={{display:"flex",gap:6,flexWrap:"wrap",borderBottom:"1.5px solid var(--border)",marginBottom:20}}>
          {TABS.map(t=>(
            <button key={t.id} onClick={()=>setTab(t.id)} style={{
              padding:"10px 16px",background:"none",border:"none",cursor:"pointer",fontFamily:"'Cairo',sans-serif",fontSize:13.5,fontWeight:700,
              color: activeTab===t.id ? COLORS.teal : COLORS.sub,
        ...
        <div className="fade-up">
          {activeTab==="overview" && (
            <div className="card" style={{padding:22,fontSize:14.5,lineHeight:2,color:COLORS.light}}>{course.description}</div>
          )}

          {activeTab==="objectives" && (
            <div className="card" style={{padding:22}}>
              <div style={{display:"flex",flexDirection:"column",gap:12}}>
                {linesToList(course.objectives).map((line,i)=>(
                  <div key={i} style={{display:"flex",gap:10,alignItems:"flex-start"}}>
                    <CheckCircle2 size={17} color={COLORS.teal} style={{flexShrink:0,marginTop:2}}/>
                    <span style={{fontSize:14,lineHeight:1.8}}>{line}</span>
                  </div>
                ))}
              </div>
            </div>
          ...
          {activeTab==="audience" && (
            <div className="card" style={{padding:22}}>
              <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(220px,1fr))",gap:12}}>
                {linesToList(course.target_audience).map((line,i)=>(
                  <div key={i} style={{display:"flex",gap:10,alignItems:"center",padding:"12px 14px",background:"var(--panel2)",borderRadius:12}}>
                    <Users size={17} color={COLORS.purple} style={{flexShrink:0}}/>
          ...
          {activeTab==="skills" && (
            <div className="card" style={{padding:22}}>
              <div style={{display:"flex",flexDirection:"column",gap:12}}>
                {linesToList(course.skills).map((line,i)=>(
                  <div key={i} style={{display:"flex",gap:10,alignItems:"flex-start"}}>
                    <Target size={17} color={COLORS.gold} style={{flexShrink:0,marginTop:2}}/>
                    <span style={{fontSize:14,lineHeight:1.8}}>{line}</span>
                  </div>
                ))}
              </div>
          ...
          {activeTab==="exam" && (
            <div className="card" style={{padding:22,fontSize:14,lineHeight:2,color:COLORS.light,whiteSpace:"pre-line"}}>{course.exam_details}</div>
          )}

          {activeTab==="syllabus" && (
            lessonsLoading ? (
              <LoadingBlock text="جاري تحميل محتوى الكورس..." />
            ) : !lessons || lessons.length===0 ? (
              <div className="card" style={{padding:24,textAlign:"center",color:COLORS.sub}}>محتوى الكورس قيد الإعداد.</div>
            ) : (
              <div className="card" style={{padding:18}}>
                <div style={{display:"flex",flexDirection:"column",gap:14}}>
                  {sectionNames.map(secName=>{
                    ...
                    return (
                      <div key={secName||"none"}>
                        {secName && <div style={{fontSize:12.5,fontWeight:700,color:COLORS.purple,marginBottom:8}}>{secName}</div>}
                        <div style={{display:"flex",flexDirection:"column",gap:6}}>
                          {secLessons.map(l=>{
                            const overallIndex = lessons.findIndex(x=>x.id===l.id)+1;
                            const isOpen = openPreviewId===l.id;
                            return (
                              <div key={l.id}>
                                <button onClick={()=> l.is_preview ? setOpenPreviewId(isOpen?null:l.id) : null} style={{
                                  width:"100%",display:"flex",alignItems:"center",gap:10,padding:"10px 10px",borderRadius:10,border:"none",
                                  cursor:l.is_preview?"pointer":"default",textAlign:"right",background: isOpen? "rgba(0,217,166,0.12)":"transparent",fontFamily:"'Cairo',sans-serif"
                                }}>
                                  {l.is_preview ? <PlayCircle size={15} color={COLORS.teal} style={{flexShrink:0}}/> : <Lock size={13} color={COLORS.sub} style={{flexShrink:0}}/>}
                                  <span style={{fontSize:13,color:COLORS.light,flex:1}}>{overallIndex}. {l.title}</span>
                                  {l.is_preview && <span style={{fontSize:10.5,color:COLORS.teal,fontWeight:700}}>معاينة مجانية</span>}
                                  <span style={{fontSize:11,color:COLORS.sub}}>{l.duration_minutes}د</span>
                                </button>
                                {isOpen && l.is_preview && (
                                  <div style={{marginTop:8,marginBottom:4,borderRadius:12,overflow:"hidden",aspectRatio:"16/9",background:"#000"}}>
                                    <iframe src={youtubeEmbedUrl(l.video_url)} title={l.title} style={{width:"100%",height:"100%",border:"none"}} allowFullScreen/>
                                  </div>
                                )}
                              </div>
                            );
