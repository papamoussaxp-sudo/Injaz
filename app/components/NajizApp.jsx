...
/* بنك أسئلة RMP بقى محفوظ في قاعدة البيانات (جدول questions) مش هنا */
...
/* بيانات المجالات لكل شهادة (مش حساسة، تفضل في الكود) — بنك الأسئلة الفعلي بقى في قاعدة البيانات */
const CERTS_WITH_CONTENT = ["PMP", "RMP"];
    ...
    <div dir="rtl" className={`theme-${theme}`} style={{minHeight:"100vh",background:COLORS.bg,fontFamily:"'Cairo',sans-serif",color:COLORS.light,transition:"background .2s ease,color .2s ease"}}>
      ...
      {view==="dashboard" && (
        unlockedCerts.length===0 ? (
          ...
          <Dashboard progress={certProgress} certCode={activeCert} questions={activeQuestions} setExamDate={(d)=>setExamDate(activeCert,d)} setView={setView} profile={profile} />
        )
      )}
      {view==="profile" && (
      ...
      {view==="practice" && (
        ...
        <Practice progress={certProgress} certCode={activeCert} recordAnswer={(qid,c,d)=>recordAnswer(activeCert,qid,c,d)} onExit={()=>setView("dashboard")} pool={activeQuestions} title="التدريب الحر" />
      )}
      {view==="review" && (
        !isUnlocked ? <PaywallScreen setView={setView} /> :
        !questionsReady ? <LoadingBlock text="جاري تحميل الأسئلة..." /> :
      ...
      {view==="mock" && (
        !isUnlocked ? <PaywallScreen setView={setView} /> :
        !questionsReady ? <LoadingBlock text="جاري تحميل الأسئلة..." /> :
        <MockExam certCode={activeCert} pool={activeQuestions} recordAnswer={(qid,c,d)=>recordAnswer(activeCert,qid,c,d)} addMockResult={(e)=>addMockResult(activeCert,e)} onExit={()=>setView("dashboard")} />
      )}
      ...
      {selectedCerts.length===0 ? (
        ...
        <div style={{display:"flex",flexDirection:"column",gap:14}}>
          {selectedCerts.map(code=>{
            ...
            const hasBank = CERTS_WITH_CONTENT.includes(code);
...
/* ============================= لوحة التحكم ============================= */
function Dashboard({ progress, certCode, questions, setExamDate, setView, profile }){
...
/* ============================= الامتحان التجريبي ============================= */
function MockExam({ certCode, pool: QUESTIONS_BANK, recordAnswer, addMockResult, onExit }){
