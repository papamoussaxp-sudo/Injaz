...
export default function NajizApp(){
  ...
  if(view==="pmp"){
    const pmpCert = certList.find(c=>c.code==="PMP");
    return (
      <div dir="rtl" className={`theme-${theme}`} style={{minHeight:"100vh",background:COLORS.bg,fontFamily:"'Cairo',sans-serif",color:COLORS.light}}>
        <FontLoader /><GlobalStyle />
        <PMPSalesPage onStart={goStart} onTrial={startTrial} cert={pmpCert} />
      </div>
    );
  }
    ...
    <div dir="rtl" className={`theme-${theme}`} style={{minHeight:"100vh",background:COLORS.bg,fontFamily:"'Cairo',sans-serif",color:COLORS.light,transition:"background .2s ease,color .2s ease"}}>
      ...
      {view==="coursePlayer" && (
        <CoursePlayer courseId={selectedCourseId} course={courses?.find(c=>c.id===selectedCourseId)}
          purchased={coursePurchases[selectedCourseId]?.status==="approved"} userId={userId} profile={profile}
...
function CourseManager(){
  ...
  const [newCourse, setNewCourse] = useState({ title:"", description:"", price_usd:79, thumbnail_url:"", gumroad_url:"" });
  ...
  const createCourse = async (e) => {
    ...
    setNewCourse({ title:"", description:"", price_usd:79, thumbnail_url:"", gumroad_url:"" });
    load();
  };
  ...
  return (
    <div>
      <form onSubmit={createCourse} className="card" style={{padding:20,marginBottom:24}}>
        ...
        <input required placeholder="اسم الكورس" value={newCourse.title} onChange={e=>setNewCourse(c=>({...c,title:e.target.value}))}
          style={{width:"100%",padding:"11px 14px",borderRadius:10,border:"1.5px solid var(--border)",background:"var(--panel2)",color:COLORS.light,fontFamily:"'Cairo',sans-serif",fontSize:14,marginBottom:12}}/>
        <textarea placeholder="وصف مختصر" value={newCourse.description} onChange={e=>setNewCourse(c=>({...c,description:e.target.value}))}
          style={{width:"100%",padding:"11px 14px",borderRadius:10,border:"1.5px solid var(--border)",background:"var(--panel2)",color:COLORS.light,fontFamily:"'Cairo',sans-serif",fontSize:14,marginBottom:12,minHeight:70}}/>
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12,marginBottom:12}}>
          <input type="number" placeholder="السعر بالدولار" value={newCourse.price_usd} onChange={e=>setNewCourse(c=>({...c,price_usd:e.target.value}))}
            style={{padding:"11px 14px",borderRadius:10,border:"1.5px solid var(--border)",background:"var(--panel2)",color:COLORS.light,fontFamily:"'Cairo',sans-serif",fontSize:14}}/>
          <input placeholder="رابط صورة الغلاف (اختياري)" value={newCourse.thumbnail_url} onChange={e=>setNewCourse(c=>({...c,thumbnail_url:e.target.value}))}
            style={{padding:"11px 14px",borderRadius:10,border:"1.5px solid var(--border)",background:"var(--panel2)",color:COLORS.light,fontFamily:"'Cairo',sans-serif",fontSize:14}}/>
        </div>
        <input placeholder="رابط منتج Gumroad" value={newCourse.gumroad_url} onChange={e=>setNewCourse(c=>({...c,gumroad_url:e.target.value}))}
          style={{width:"100%",padding:"11px 14px",borderRadius:10,border:"1.5px solid var(--border)",background:"var(--panel2)",color:COLORS.light,fontFamily:"'Cairo',sans-serif",fontSize:14,marginBottom:14}}/>
        <button className="btn btn-primary" type="submit" disabled={savingCourse}>{savingCourse?"جاري الإنشاء...":"إنشاء الكورس"}</button>
      </form>
            ...
            <div key={c.id} className="card" style={{padding:18}}>
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",flexWrap:"wrap",gap:10}}>
                <div>
                  ...
                  <div style={{fontSize:12,color:COLORS.sub}}>${c.price_usd} · {c.status==="published"?"منشور":"مسودة"}</div>
                </div>
                <div style={{display:"flex",gap:8}}>
                  <button className="btn btn-outline" style={{fontSize:12,padding:"6px 12px"}} onClick={()=>togglePublish(c)}>
                    {c.status==="published" ? "إخفاء" : "نشر"}
                  </button>
                  <button className="btn" style={{fontSize:12,padding:"6px 12px",background:"var(--panel2)"}} onClick={()=>setExpandedId(expandedId===c.id?null:c.id)}>
...
function LessonManager({ courseId }){
  const [lessons, setLessons] = useState(null);
  const [newLesson, setNewLesson] = useState({ title:"", section:"", video_url:"", duration_minutes:10, is_preview:false, pdf_url:"" });
  const [saving, setSaving] = useState(false);
  ...
  const uploadPdf = async (file) => {
    ...
    if(error){ alert("تعذر رفع الملف: "+error.message); return; }
    setNewLesson(l=>({...l, pdf_url: path}));
  };
  ...
  const addLesson = async (e) => {
    ...
    setNewLesson({ title:"", section: newLesson.section, video_url:"", duration_minutes:10, is_preview:false, pdf_url:"" }); // نسيب اسم القسم زي ما هو عشان تضيف دروس تانية في نفس القسم بسرعة
    load();
  };
  ...
  return (
    <div style={{marginTop:16,paddingTop:16,borderTop:"1px solid var(--border)"}}>
      <form onSubmit={addLesson} style={{display:"flex",flexDirection:"column",gap:8,marginBottom:16}}>
        <input placeholder="اسم القسم (مثال: People / Process / Business Environment) — اختياري" value={newLesson.section} onChange={e=>setNewLesson(l=>({...l,section:e.target.value}))}
          style={{padding:"9px 12px",borderRadius:9,border:"1.5px solid var(--border)",background:"var(--panel2)",color:COLORS.light,fontFamily:"'Cairo',sans-serif",fontSize:13}}/>
        <input required placeholder="اسم الدرس" value={newLesson.title} onChange={e=>setNewLesson(l=>({...l,title:e.target.value}))}
          style={{padding:"9px 12px",borderRadius:9,border:"1.5px solid var(--border)",background:"var(--panel2)",color:COLORS.light,fontFamily:"'Cairo',sans-serif",fontSize:13}}/>
        <input required placeholder="رابط فيديو يوتيوب" value={newLesson.video_url} onChange={e=>setNewLesson(l=>({...l,video_url:e.target.value}))}
          style={{padding:"9px 12px",borderRadius:9,border:"1.5px solid var(--border)",background:"var(--panel2)",color:COLORS.light,fontFamily:"'Cairo',sans-serif",fontSize:13}}/>
        <label className="btn btn-outline" style={{fontSize:12.5,cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center",gap:6,opacity:uploadingPdf?0.6:1}}>
          ...
          {uploadingPdf ? "جاري الرفع..." : newLesson.pdf_url ? "✓ ملف PDF مرفوع — دوس لتغييره" : "ارفع ملف PDF مرفق (اختياري)"}
        ...
        <div style={{display:"flex",gap:10,alignItems:"center"}}>
          <input type="number" placeholder="المدة بالدقايق" value={newLesson.duration_minutes} onChange={e=>setNewLesson(l=>({...l,duration_minutes:e.target.value}))}
            style={{flex:1,padding:"9px 12px",borderRadius:9,border:"1.5px solid var(--border)",background:"var(--panel2)",color:COLORS.light,fontFamily:"'Cairo',sans-serif",fontSize:13}}/>
          ...
          <button className="btn btn-primary" type="submit" disabled={saving} style={{fontSize:12.5,padding:"9px 16px"}}>{saving?"...":"إضافة"}</button>
        </div>
      </form>
      ...
      {lessons && lessons.length>0 && (
        <div style={{display:"flex",flexDirection:"column",gap:6}}>
          {lessons.map((l,i)=>(
            <div key={l.id} style={{display:"flex",alignItems:"center",gap:10,padding:"8px 10px",background:"var(--panel2)",borderRadius:8}}>
              <span style={{fontSize:12.5,flex:1}}>
                {l.section && <span style={{color:COLORS.purple,fontWeight:700}}>[{l.section}] </span>}
                {i+1}. {l.title} {l.is_preview && <span style={{color:COLORS.teal,fontSize:10.5}}>(معاينة)</span>} {l.pdf_url && <FileCheck2 size={11} style={{verticalAlign:"middle",marginRight:4}} color={COLORS.teal}/>}
              </span>
...
/* ============================= صفحة مبيعات بنك أسئلة PMP المخصصة ============================= */
function PMPSalesPage({ onStart, onTrial, cert }){
  const price = cert?.price_usd || 79;
  const scrollToBuy = () => {
    const el = document.getElementById("pmp-buy-section");
    ...
    <div>
      ...
      <div style={{position:"sticky",top:0,zIndex:20,background:"var(--navbg)",backdropFilter:"blur(10px)",borderBottom:"1px solid var(--border)"}}>
        <div style={{maxWidth:1000,margin:"0 auto",display:"flex",alignItems:"center",justifyContent:"space-between",padding:"12px 20px"}}>
          <Logo />
          <button className="btn btn-primary" style={{padding:"8px 18px",fontSize:13.5}} onClick={scrollToBuy}>اشترِ الآن — ${price}</button>
        </div>
      </div>
      ...
      <div style={{position:"relative",overflow:"hidden"}}>
        ...
        <div style={{maxWidth:820,margin:"0 auto",padding:"56px 20px 20px",textAlign:"center",position:"relative"}}>
          <div style={{display:"inline-flex",alignItems:"center",gap:8,background:"rgba(255,200,87,0.1)",border:"1px solid rgba(255,200,87,0.3)",borderRadius:20,padding:"6px 16px",fontSize:13,color:COLORS.gold,marginBottom:22}}>
            <Sparkles size={14}/> متوافق بالكامل مع أحدث تحديث لمخطط الامتحان (ECO 2026)
          </div>
          <h1 style={{fontSize:"clamp(28px,5vw,44px)",fontWeight:800,lineHeight:1.35,margin:"0 0 18px"}}>
            بنك أسئلة PMP<br/>
            <span style={{background:`linear-gradient(90deg, var(--teal), var(--gold))`,WebkitBackgroundClip:"text",WebkitTextFillColor:"transparent"}}>الأقوى بالعربي — كامل</span>
          </h1>
          <p style={{color:COLORS.sub,fontSize:16,lineHeight:2,maxWidth:600,margin:"0 auto 30px"}}>
            مش مجرد أسئلة حفظ. بنك أسئلة موقفي (Situational) بيحاكي صعوبة الامتحان الفعلي، بشرح تفصيلي لكل إجابة، ونظام بيكتشف نقاط ضعفك أول بأول.
          </p>
          <div style={{display:"flex",gap:14,justifyContent:"center",flexWrap:"wrap",marginBottom:20}}>
            <button className="btn btn-primary" style={{padding:"14px 30px",fontSize:16}} onClick={onTrial}>جرّب 10 أسئلة مجانًا الآن</button>
            <button className="btn btn-outline" style={{padding:"14px 30px",fontSize:16}} onClick={scrollToBuy}>اشترِ البنك كامل — ${price}</button>
          </div>
          <div style={{fontSize:12.5,color:COLORS.sub}}>بدون تسجيل • بدون بطاقة ائتمان • نتيجتك فورًا</div>
        </div>
      </div>

      {/* المشكلة */}
      <div style={{maxWidth:820,margin:"0 auto",padding:"50px 20px 10px"}}>
        <div className="card" style={{padding:28}}>
          <div style={{fontWeight:800,fontSize:20,marginBottom:16,textAlign:"center"}}>ليه معظم اللي بيذاكروا PMP بيحسّوا إنهم تايهين؟</div>
          <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(230px,1fr))",gap:14}}>
            {[
              "مصادر بالإنجليزي البحت، صعبة الفهم في أول قراءة",
              "أسئلة سطحية مالهاش علاقة بأسلوب الامتحان الفعلي الموقفي",
              "مفيش شرح حقيقي — بس \"الإجابة الصح كذا\" من غير سبب",
              "مفيش أي فكرة عن نقاط ضعفك الحقيقية قبل ما تدخل الامتحان",
            ].map((t,i)=>(
              <div key={i} style={{display:"flex",gap:10,alignItems:"flex-start",fontSize:13.5,color:COLORS.sub,lineHeight:1.8}}>
                <X size={16} color="#ff8a8a" style={{flexShrink:0,marginTop:2}}/>
                <span>{t}</span>
              </div>
            ))}
          </div>
      ...
      {/* المميزات بالتفصيل */}
      <div style={{maxWidth:1000,margin:"0 auto",padding:"50px 20px 10px"}}>
        <div style={{textAlign:"center",marginBottom:36}}>
          <div style={{fontWeight:800,fontSize:26}}>إيه اللي بتاخده بالظبط</div>
          <div style={{color:COLORS.sub,marginTop:8}}>مش مجرد "أسئلة" — نظام تحضير متكامل</div>
        </div>
        <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(270px,1fr))",gap:16}}>
          {[
            { icon:Target, color:COLORS.teal, title:"أسئلة موقفية حقيقية", text:"كل سؤال سيناريو واقعي بمنطق الامتحان الفعلي، مش سؤال حفظ مباشر." },
            { icon:ListChecks, color:COLORS.gold, title:"شرح تفصيلي لكل إجابة", text:"مش بس الإجابة الصح — هتفهم ليه باقي الاختيارات غلط كمان." },
            { icon:Brain, color:COLORS.purple, title:"كشف نقاط الضعف تلقائيًا", text:"أي سؤال تغلط فيه يتسجل في سجل مراجعة خاص بيك، تعيد التدرب عليه لحد ما تتقنه." },
            { icon:Clock, color:"#ff8a5c", title:"امتحان تجريبي بمؤقت حقيقي", text:"محاكاة فعلية لظروف الاختبار، بنفس توزيع أوزان النطاقات الرسمية." },
            { icon:TrendingUp, color:COLORS.teal, title:"نظام تركيز ذكي", text:"بيحلل أداءك الفعلي ويقترح عليك أنهي نطاق تركّز عليه الأسبوع ده." },
            { icon:RotateCcw, color:COLORS.gold, title:"بنك بيتحدث باستمرار", text:"إضافات دورية لأسئلة جديدة من غير أي رسوم إضافية على اللي دافع بالفعل." },
          ].map((f,i)=>{
            const Icon = f.icon;
            return (
              <div key={i} className="card" style={{padding:22}}>
                <div style={{width:44,height:44,borderRadius:12,background:`${f.color}22`,display:"flex",alignItems:"center",justifyContent:"center",marginBottom:14}}>
                  <Icon size={21} color={f.color}/>
                </div>
                <div style={{fontWeight:700,fontSize:15,marginBottom:6}}>{f.title}</div>
                <div style={{fontSize:13,color:COLORS.sub,lineHeight:1.8}}>{f.text}</div>
              </div>
            );
          })}
        </div>
      ...
      {/* تجربة مجانية CTA */}
      <div style={{maxWidth:700,margin:"0 auto",padding:"50px 20px 10px"}}>
        <div className="card" style={{padding:30,textAlign:"center",background:"rgba(0,217,166,0.06)",border:"1px solid rgba(0,217,166,0.25)"}}>
          <Sparkles size={26} color={COLORS.teal} style={{marginBottom:10}}/>
          <div style={{fontWeight:800,fontSize:19,marginBottom:8}}>مش لازم تصدّقنا على الكلام — جرّب بنفسك</div>
          <div style={{color:COLORS.sub,fontSize:13.5,marginBottom:20}}>10 أسئلة حقيقية من نفس مستوى البنك، من غير تسجيل ولا بطاقة ائتمان</div>
          <button className="btn btn-primary" style={{padding:"12px 28px"}} onClick={onTrial}>ابدأ التجربة المجانية الآن</button>
        </div>
      </div>
      ...
      <div style={{maxWidth:820,margin:"0 auto",padding:"50px 20px 10px"}}>
        <div className="card" style={{padding:28}}>
          <div style={{fontWeight:700,fontSize:16,marginBottom:18,textAlign:"center"}}>البنك موزّع بنفس أوزان الامتحان الرسمية</div>
          <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:14}}>
            {[["الأفراد (People)","33%",COLORS.purple],["العمليات (Process)","41%",COLORS.teal],["بيئة العمل (Business)","26%",COLORS.gold]].map(([label,pct,color],i)=>(
              <div key={i} style={{textAlign:"center",padding:"16px 8px",background:"var(--panel2)",borderRadius:12}}>
                <div style={{fontSize:24,fontWeight:800,color}}>{pct}</div>
                <div style={{fontSize:11.5,color:COLORS.sub,marginTop:4}}>{label}</div>
              </div>
            ))}
          </div>
        </div>
      ...
      <div style={{maxWidth:1000,margin:"0 auto",padding:"50px 20px 10px"}}>
        <div style={{textAlign:"center",marginBottom:30}}>
          <div style={{fontWeight:800,fontSize:24}}>تجارب حقيقية</div>
        </div>
        <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(240px,1fr))",gap:16}}>
          {TESTIMONIALS.map((t,i)=>(
            <div key={i} className="card" style={{padding:20}}>
              <div style={{display:"flex",gap:2,marginBottom:10}}>
                {Array.from({length:5}).map((_,s)=><Star key={s} size={13} fill={s<t.stars?"#FFC857":"none"} color="#FFC857"/>)}
              </div>
              <div style={{fontSize:13,color:COLORS.sub,lineHeight:1.8,marginBottom:14}}>{t.text}</div>
              <div style={{fontSize:12.5,fontWeight:700}}>{t.name} <span style={{color:COLORS.sub,fontWeight:500}}>· {t.city}</span></div>
            </div>
          ))}
        </div>
      </div>
      ...
      {/* الضمان */}
      <div style={{maxWidth:820,margin:"0 auto",padding:"50px 20px 10px"}}>
        <div className="card" style={{padding:30,background:"rgba(255,200,87,0.06)",border:"1px solid rgba(255,200,87,0.25)"}}>
          <div style={{display:"flex",gap:14,alignItems:"flex-start"}}>
            <Shield size={30} color={COLORS.gold} style={{flexShrink:0}}/>
            <div>
              <div style={{fontWeight:800,fontSize:18,marginBottom:8}}>ضمان النجاح معنا</div>
              <div style={{fontSize:13.5,color:COLORS.sub,lineHeight:2}}>
                في حال عدم اجتياز اختبار PMP الفعلي بعد الانتهاء من جميع الأسئلة والمذاكرة الكاملة داخل المنصة،
                يمكنك طلب تمديد وصولك للبنك مرة أخرى من غير أي رسوم إضافية، لحد ما تنجح.
              </div>
            </div>
          </div>
        </div>
      ...
      {/* FAQ */}
      <div style={{maxWidth:820,margin:"0 auto",padding:"50px 20px 20px"}}>
        <div style={{textAlign:"center",marginBottom:30}}>
          <div style={{fontWeight:800,fontSize:24}}>أسئلة شائعة عن البنك</div>
        </div>
        <FAQAccordion items={[
          { q:"الأسئلة دي مسرّبة من الامتحان الفعلي؟", a:"لأ. PMI بيمنع نشر أي أسئلة حقيقية من الامتحان قانونيًا. أسئلتنا مؤلَّفة بالكامل بأسلوب موقفي مطابق لمنطق الامتحان الفعلي، مبني على أحدث تحديث لمخطط المحتوى (ECO 2026)." },
          { q:"البنك فيه كام سؤال بالظبط؟", a:"البنك في نمو مستمر وبيتحدث بانتظام بأسئلة جديدة من غير أي رسوم إضافية على مين اشترى بالفعل." },
          { q:"هل ينفع أذاكر بيه من الموبايل؟", a:"أيوه، المنصة مصممة تشتغل بنفس الجودة على الموبايل والكمبيوتر." },
          { q:"لو معنديش وسيلة دفع تقبل بطاقتي؟", a:"بوابة الدفع بتقبل فيزا وماستركارد الدولية (ومعظم بطاقات مدى المزدوجة). تواصل معانا لو واجهت مشكلة." },
          { q:"إمتى هيتفعّل حسابي بعد الدفع؟", a:"بعد الشراء وترفع إيصالك من ملفك الشخصي، بيتفعّل خلال ساعات قليلة." },
        ]}/>
      </div>

      {/* الشراء */}
      <div id="pmp-buy-section" style={{maxWidth:600,margin:"0 auto",padding:"20px 20px 70px"}}>
        <div className="card" style={{padding:32,textAlign:"center"}}>
          <div style={{fontWeight:800,fontSize:20,marginBottom:6}}>ابدأ رحلتك نحو النجاح من أول محاولة</div>
          <div style={{fontSize:34,fontWeight:800,margin:"14px 0"}}>${price}</div>
          <div style={{fontSize:12.5,color:COLORS.sub,marginBottom:22}}>دفعة واحدة — وصول كامل لبنك أسئلة PMP</div>
          <button className="btn btn-primary" style={{width:"100%",padding:"14px",fontSize:16,marginBottom:10}} onClick={onStart}>سجّل حساب وابدأ الشراء</button>
          <button className="btn btn-outline" style={{width:"100%",padding:"12px"}} onClick={onTrial}>جرّب مجانًا الأول</button>
        </div>
      </div>
    </div>
  );
...
/* ============================= مشغّل الكورس ============================= */
function CoursePlayer({ courseId, course, purchased, userId, profile, onExit }){
        ...
        <div style={{display:"grid",gridTemplateColumns:"2fr 1fr",gap:20}}>
          <div>
            ...
            <div style={{display:"flex",gap:10,flexWrap:"wrap"}}>
              {(activeLesson.is_preview || purchased) && (
                <button className="btn btn-outline" onClick={()=>markDone(userId, activeLesson.id)} disabled={!!done[activeLesson.id]}>
