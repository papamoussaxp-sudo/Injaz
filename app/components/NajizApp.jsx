      ...
      {view==="coursePlayer" && (
        coursePurchases[selectedCourseId]?.status==="approved" ? (
          ...
          <CourseDetailsPage
            ...
            userId={userId} isLoggedIn={!!session} purchase={coursePurchases[selectedCourseId]}
...
function CourseDetailsPage({ course, courseId, userId, isLoggedIn, purchase, onPurchaseAdded, onStartWatching, onLogin, onBack }){
  ...
  if(!course){
    return <LoadingBlock text="جاري تحميل تفاصيل الكورس..." />;
  }
  ...
  return (
    <div style={{maxWidth:1000,margin:"0 auto",padding:"24px 20px 60px"}}>
      <button onClick={onBack} style={{background:"none",border:"none",color:COLORS.sub,fontFamily:"'Cairo',sans-serif",cursor:"pointer",fontSize:13,marginBottom:16}}>← رجوع</button>
        ...
        <div className="card" style={{padding:18,minWidth:220,textAlign:"center"}}>
          {isApproved ? (
            ...
            <>
              ...
              {!isLoggedIn ? (
                <button className="btn btn-primary" style={{width:"100%"}} onClick={onLogin}>سجّل دخولك للشراء</button>
              ) : (
                <div style={{display:"flex",flexDirection:"column",gap:8}}>
                  {course.gumroad_url && (
                    <a href={course.gumroad_url} target="_blank" rel="noopener noreferrer" className="btn btn-gold" style={{textDecoration:"none",fontSize:14}}>
                      <ShoppingCart size={15} style={{verticalAlign:"middle",marginLeft:6}}/> اشترِ الآن — أضِفه لباقتك
                    </a>
                  )}
